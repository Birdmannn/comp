...
pub mod SalesComponent {
    ...
    use crate::interfaces::sales::{BuyEvent, ISales, SalesStatus, SellEvent};
    ...
    > of ISales<ComponentState<TContractState>> {
        fn buy(ref self: ComponentState<TContractState>, item_id: u256) {
            ...
            // Emit buy event
            self.emit(Event::Bought(BuyEvent { id: item_id, buyer: get_caller_address() }));
        }
        ...
        fn sell(ref self: ComponentState<TContractState>, item_id: u256) {
            ...
            // Emit sell event👀
            self.emit(Event::Sold(SellEvent { id: item_id, seller: caller }));
        }
